...
void UrlPath::appendSubnameEncode(std::string &subname, std::string &url_path)
{
 ...
 * @return 0:成功  -1:失敗
**/
int UrlPath::getBaseDir(std::string &basedir, std::string &dir_name)
{
    int ret = -1;
 ...
 * ファイルパスからURLパスを求める
**/
void UrlPath::encode(std::string &url_path, std::string &file_path)
{
...
/**
 * rootパラメータの実体パスを返さないでエンコードする
**/
int UrlPath::getEncodedPath(std::string &encoded_path, std::string &root, std::string &url_path)
{
    std::string root_path;
    std::string chop, canonicalized;

    if (getBaseDir(root_path, root) != 0)
    {
        // no such directory
        return -1;
    }

    if (futil->isTraversalPath(url_path) == true)
    {
        err_message = "invalid URL path";
        return -2;
    }

    if (!url_path.empty() && url_path.find(root_path) != 0)
    {
        err_message = "root or url_path is missing";
        return -3;
    }

    chop = url_path.substr(root_path.length(), url_path.length() - root_path.length());
    futil->getCanonicalizePath(canonicalized, chop);

    encode(encoded_path, canonicalized);
    return 0;
}
 ...
 * URLパスからファイルパスを求める
**/
void UrlPath::decode(std::string &file_path, std::string &url_path)
{
    std::string split_name;
    std::string::size_type start_pos, end_pos;
...
int UrlPath::getDecodedPath(std::string &decoded_path, std::string &root, std::string &url_path)
{
    std::string path;
    std::string decoded;
